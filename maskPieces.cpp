...
char output[20];
sprintf(output, "PieceMasks/m_%s.jpg",square);
Mat stackedMask;
bool first = true;
...
char input[20];
sprintf(input, "IsolatedPieces/o_%s_%s_%s_%d.jpg",square,color,piece,l);
printf("%s\n",input);
    ...
    if( first) {
        ...
        image.copyTo(stackedMask,image);
    }
    namedWindow(input, WINDOW_NORMAL);
    imshow(input, stackedMask);
}
}
}
	imwrite(output,stackedMask);
}
