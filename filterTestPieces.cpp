...
char input[20];
sprintf(input, "TestData/%d.jpg",t);
printf("%s\n",input);
...
char squareMask[20];
sprintf(squareMask, "PieceMasks/m_%s.jpg",square);

char output[20];
sprintf(output, "IsolatedTestData/i_%d_%s.jpg",t,square);

    Mat image, mask, brd;
    image = imread(input, CV_LOAD_IMAGE_COLOR);   // Read the file
    mask = imread(squareMask, CV_LOAD_IMAGE_COLOR);   // Read the file
    ...
    image.copyTo(cutImage, mask);
    brd.copyTo(cutBrd, mask);
