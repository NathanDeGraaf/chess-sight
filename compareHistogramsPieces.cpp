...
char input[20];
sprintf(input, "IsolatedTestData/i_%d_%s.jpg",t,square);
printf("%s\n",input);


for(int j = 0 ; j < 2 ; j++) { // colors
char* color = colors[j];
printf("assuming color is: %s\n",color);


char pieceDataString[20];
sprintf(pieceDataString, "PieceHistograms/%s_%s",square,color);
...
char pFile[20];
sprintf(pFile,"%s_P.txt",pieceDataString);
char bFile[20];
sprintf(bFile,"%s_B.txt",pieceDataString);
char rFile[20];
sprintf(rFile,"%s_R.txt",pieceDataString);
char qFile[20];
sprintf(qFile,"%s_Q.txt",pieceDataString);
char kFile[20];
sprintf(kFile,"%s_K.txt",pieceDataString);
char nFile[20];
sprintf(nFile,"%s_N.txt",pieceDataString);

     double ** pawn = readFromFile(pFile, histSize);
     double ** bishop = readFromFile(bFile, histSize);
     double ** rook = readFromFile(rFile, histSize);
     double ** queen = readFromFile(qFile, histSize);
     double ** king = readFromFile(kFile, histSize);
     double ** knight = readFromFile(nFile, histSize);
